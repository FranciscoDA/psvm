...
do
	...
	do
		...
		do
			./cusvm --train-attributes mnist/train-images.idx3-ubyte --train-labels mnist/train-labels.idx1-ubyte --train-format idx --train-n 15000 --test-attributes mnist/t10k-images.idx3-ubyte --test-labels mnist/t10k-labels.idx1-ubyte --test-format idx --test-n $tn --normalize-zero-one --cost $C --kernel rbf --gamma $gamma > "results/C_${C}_tn_${tn}_rbf_gamma_${gamma}.txt"
		done
		for p in 2 4 6 9
		do
			for c in 0 1
